...
- **Project pane:** full frame, schematic split - 7fr plate visual (dot-grid + monogram + mono caption) over raised body (title, description, sharp 4px chips, CTA, mono status). Two panes: the flagship (DentVault, live) plus a flipped variant (`5fr/7fr`, visual on the right) for DentVault Mobile so the pair never reads as a clone row. Real product screenshot preferred; typographic plate is the acceptable interim (TODO in markup).
